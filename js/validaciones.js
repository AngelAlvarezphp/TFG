...
function validarMascota(
  ...
  if (!idValido) {
    errores.push(
      "El identificador de la mascota debe estar formado por 8 numeros"
    );
...
function validarCitas() {
  ...
  const descripcionValido = inputDescripcion.value.match(
    patterns.empiezaMayuscula
  );
  let errores = [];
  if (!idValido) {
...
function attachValidationsToForms() {
...
document.addEventListener("DOMContentLoaded", attachValidationsToForms);
